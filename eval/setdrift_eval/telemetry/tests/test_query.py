...
def test_events_per_session(tmp_path, monkeypatch):
    ...
    _write_session(tmp_path, "big", [{"_session": "big", "tool_name": "Bash", "_ts_captured": _now()} for _ in range(3)])
    _write_session(tmp_path, "small", [{"_session": "small", "tool_name": "Read", "_ts_captured": _now()}])
...
def test_missing_canary_detects_absence(tmp_path, monkeypatch):
    ...
    _write_session(tmp_path, "sY", [{"_session": "sY", "tool_name": "Bash", "_ts_captured": _now()}])
    q = _query(tmp_path, monkeypatch)
...
def test_parquet_promotion_threshold(tmp_path, monkeypatch):
    ...
    _write_session(tmp_path, "s1", [{"_session": "s1", "tool_name": "Bash", "_ts_captured": _now()}])
    q = _query(tmp_path, monkeypatch)
