...
from setdrift_eval.drift.grid_runner import (
    ARMS,
    GridRunnerError,
    MODELS,
    N_RUNS,
...
def test_grid_runner_imports_frozen_ruler() -> None:
    ...
    assert "noise_band" in scorer_imports, (
        "grid_runner.py must import noise_band from setdrift_eval.telemetry.scorer "
        "(FROZEN RULER — never re-implement)"
    )
    assert "macro_f1" in scorer_imports, (
        "grid_runner.py must import macro_f1 from setdrift_eval.telemetry.scorer "
        "(FROZEN RULER — never re-implement)"
    )
