...
def test_test_partition_filter_excludes_val(tmp_path: Path) -> None:
    ...
    with (
        patch("setdrift_eval.drift.haiku_arm.run_arm", side_effect=_fake_run_arm),
        patch("setdrift_eval.drift.haiku_arm.load_or_call", side_effect=_fake_load_or_call),
    ):
...
def test_report_contains_5run_band_per_arm(tmp_path: Path) -> None:
    ...
    from setdrift_eval.drift import haiku_arm
    from setdrift_eval.telemetry.scorer import noise_band as real_noise_band
