...
def _make_d4j_fixture(tmp_path: Path, bugs=(1, 2)) -> Path:
    ...
    (root / "commit-db").write_text("".join(f"{b},aaa{b},bbb{b}\n" for b in bugs), encoding="utf-8")
    for b in bugs:
...
def test_multi_source_build_includes_defects4j(tmp_path: Path):
    ...
    corpus = build_corpus(raw_dir=raw_dir, output_path=out, corpus_version="t", defects4j_dir=d4j)
...
def test_negative_topup_reaches_floor(tmp_path: Path):
    ...
    corpus = build_corpus(
        ...
        output_path=out,
        corpus_version="t",
        min_negative_fraction=0.20,
        constructed_negatives_path=tsv,
    )
