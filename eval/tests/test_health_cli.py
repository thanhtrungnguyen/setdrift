...
_FULL_MAP = {
    ...
    "jpa-migration": [], "dependency-bump": [], "null-check": [],
    "test-fixture-fix": [], "config-property": [], "import-fix": [], "none": [],
}
...
def _stage(tmp_path, monkeypatch, *, corpus_rows, splits, passed=True, per_source=None):
    ...
    monkeypatch.setattr(arm_runner, "run_arm_c", lambda prompt, **kw: (seen.append(prompt), set())[1])
    return cp, mp, exp, seen


_FULL_PS = {"gitbug-java": {"precision": 0.9, "kappa": 0.7, "n": 10, "label_distribution": {"none": 8}}}


def _row(pid, prompt, gt, source="gitbug-java"):
    return {
        "prompt_id": pid, "prompt": prompt,
        "predicted_skills": gt, "ground_truth_skills": gt,
...
def test_gate_guard_raises_without_passed_report(tmp_path, monkeypatch):
    cp, mp, exp, _ = _stage(
        ...
        splits={"p1": "test"}, passed=False,  # gate not cleared
    )
    with pytest.raises(ScorerError, match="gate has not cleared"):
        run_health(cp, "B", map_path=mp, skills_dir=tmp_path, experiments_dir=exp, cache_dir=tmp_path / "cache")
...
def test_per_source_kappa_failloud_on_empty(tmp_path, monkeypatch):
    cp, mp, exp, _ = _stage(
        tmp_path, monkeypatch,
        corpus_rows=[_row("p1", "add endpoint", ["spring-annotation-fix"])],
        splits={"p1": "test"}, passed=True, per_source={},  # passed but no per-source metrics
    )
...
def test_holdout_excluded_not_negative(tmp_path, monkeypatch):
    ...
    cp, mp, exp, _ = _stage(tmp_path, monkeypatch, corpus_rows=rows, splits={"p1": "val", "p2": "val", "p3": "test"})
    run_health(cp, "B", map_path=mp, skills_dir=tmp_path, experiments_dir=exp,
...
def test_scores_val_test_only_not_train(tmp_path, monkeypatch):
    ...
    cp, mp, exp, seen = _stage(tmp_path, monkeypatch, corpus_rows=rows, splits={"p_train": "train", "p_val": "val"})
    run_health(cp, "B", map_path=mp, skills_dir=tmp_path, experiments_dir=exp,
               cache_dir=tmp_path / "cache", timestamp="2026-06-01T00:00:00Z")
...
def test_arm_c_floor_and_results_fields(tmp_path, monkeypatch):
    ...
    cp, mp, exp, _ = _stage(tmp_path, monkeypatch, corpus_rows=rows, splits={"p1": "val", "p2": "test"})
    result = run_health(cp, "C", map_path=mp, skills_dir=tmp_path, experiments_dir=exp,
                        cache_dir=tmp_path / "cache", timestamp="2026-06-01T00:00:00Z")
    assert result.arm == "C"
    manifest = json.loads(next(exp.glob("*-results.json")).read_text(encoding="utf-8"))
    for field in (
        "intent_map_sha256", "split_hash", "macro_f1_runs", "bootstrap_ci_low",
        "token_cost_total", "per_source_kappa", "per_source_label_distribution",
        "coverage_pct", "n_prompts_out_of_coverage",
    ):
...
def test_cli_arm_required_and_prints(tmp_path, monkeypatch, capsys):
    ...
    cp, mp, exp, _ = _stage(
        tmp_path, monkeypatch,
        corpus_rows=[_row("p1", "add endpoint", ["spring-annotation-fix"]), _row("p2", "x", ["none"])],
