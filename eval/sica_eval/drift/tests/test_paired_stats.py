...
def test_paired_diff_result_reason_always_populated(tmp_path: Path) -> None:
    ...
    from sica_eval.drift.paired_stats import PairedDiffResult
...
def test_paired_stats_imports_frozen_noise_band() -> None:
    """Test 5: paired_stats.py imports noise_band from sica_eval.telemetry.scorer (FROZEN RULER)."""
    import ast
    ...
    # Must import noise_band from sica_eval.telemetry.scorer
    ...
    assert "noise_band" in scorer_imports, (
        "paired_stats.py must import noise_band from sica_eval.telemetry.scorer "
...
def test_exceeds_b_upper_band_logic(tmp_path: Path) -> None:
    ...
    from sica_eval.drift.paired_stats import paired_difference_report
    from sica_eval.telemetry.scorer import noise_band as frozen_noise_band
