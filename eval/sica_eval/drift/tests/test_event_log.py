...
def test_drift_event_validates():
    ...
    from sica_eval.drift.event_log import DriftEvent
...
def test_scrub_for_public_has_no_commit_message():
    ...
    from sica_eval.drift.event_log import DriftEvent, scrub_for_public
...
def test_append_drift_event_dual_writes(tmp_path):
    ...
    from sica_eval.drift.event_log import DriftEvent, append_drift_event
...
def test_bank_transcripts_produces_cache_hit(tmp_path, monkeypatch):
    ...
    from sica_eval.benchmark.response_cache import cache_key, CACHE_DIR
    ...
    monkeypatch.setattr(
        "sica_eval.benchmark.response_cache.load_or_call",
        fake_load_or_call,
    )
