...
from sica_eval.optimizer.applier import apply_proposal as _apply_proposal
from sica_eval.optimizer.applier import stage_signed_candidate as _stage_signed_candidate
from sica_eval.optimizer.gepa_wrapper import propose as _propose
from sica_eval.optimizer.gepa_wrapper import build_optimizer_trainset as _build_optimizer_trainset
from sica_eval.optimizer.signer import sign_config
from sica_eval.optimizer.verifier import verify_candidate as _verify_candidate
...
def _check_hmac_key() -> None:
    ...
    from sica_eval.optimizer.signer import SigningKeyError, _load_key

    try:
        _load_key()
    except SigningKeyError as exc:
        raise OrchestratorError(
            f"HMAC signing key not found — run 'sica-eval init-keys' first. "
...
def _load_trainset(corpus_path: Path, map_path: Path) -> tuple[list, dict]:
    ...
    """
    import yaml
    from sica_eval.telemetry.scorer import load_intent_map
    ...
    if not split_path.exists():
        raise OrchestratorError(
            ...
            "Run 'sica-eval corpus build' to generate the train/val/test split first."
        )
...
def run_loop_cycle(
    ...
    *,
    ...
    from sica_eval.telemetry.scorer import scored_intents
